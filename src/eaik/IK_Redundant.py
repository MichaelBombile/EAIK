...
class SearchableRedundantUrdfRobot(IKRobot):
    ...
    def _search_values(self, joint_index: int, center: float | None = None, span: float | None = None):
        ...
        if center is None:
            ...
            values = np.linspace(max(lower, center - half_span), min(upper, center + half_span), self._refinement_grid_size)

        if lower <= 0.0 <= upper:
            values = np.unique(np.concatenate([values, np.array([0.0])]))
        return values

    def _score_solution(self, pose: np.ndarray, solution_row: np.ndarray):
        error = np.linalg.norm(self.fwdKin(solution_row) - pose)
        return float(error)
    ...
    def IK(self, pose: np.ndarray):
        ...
        for candidate in self._candidate_info:
            ...
            lower, upper = self._joint_limits[joint_index]
            total_span = (upper - lower) if np.isfinite(lower) and np.isfinite(upper) and upper > lower else 2.0 * np.pi
            trial_centers = [(None, None)]

            for center, span in trial_centers:
                values = self._search_values(joint_index, center, span)
                local_best_q = None
                local_best_err = np.inf

                for q in values:
                    bot = self._get_robot(self._fixed_axes + [(joint_index, float(q))])
                    ...
                    for row, is_ls in zip(np.asarray(sols.Q), np.asarray(sols.is_LS, dtype=bool)):
                        err = self._score_solution(pose, row)
                        if err < local_best_err:
                            local_best_err = err
                            local_best_q = float(q)
                        self._append_unique_solution(seen_rows, seen_ls, np.asarray(row, dtype=np.float64), bool(is_ls))
                        ranked_rows.append((bool(is_ls), err, np.asarray(row, dtype=np.float64)))
        ...
        for is_ls, _, row in ranked_rows:
            ...
            self._append_unique_solution(ordered_rows, ordered_ls, row, is_ls)
            if len(ordered_rows) == before:
                continue
